...
def player_attack(player,enemy,zonemap):
    if not player.weapon.broken:
        ...
        broken_damage = math.ceil(player.weapon.damage * 0.1) #Immer aufgerundet (aka 1,2 wird 2), 10% Waffenschaden falls kaputt
...
# This is where the magic happens... -----------------------------------------
        if player.mp_cur_max[0] >= cast_spell.mana_cost: #genug mana?
            ...
            print("You don't have enough mana, you cast " + random.choice(["butterflies", "bubbles", "hot air", "a flower"]) + " instead.")
            fight(player,enemy,zonemap)
    else:
...
def prompt():
    ...
    acceptable_locations = ["move", "go", "travel", "walk", "quit", "examine", "inspect", "interact", "look", "hunting",
                            ...
                            "use potion", "show inventory", "inventory", "show stats", "stats", "buy", "sell", "blacksmith", "knock","magic","learn","spell"]
