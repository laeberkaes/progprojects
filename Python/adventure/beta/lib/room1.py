...
from lib.static import clear, speech_manipulation, confirmation
...
class Room1:
    ...
    def intro(self, player):
        ...
        if ant not in ["yes", "y"]:
            ...
            speech_manipulation("Thank you!!", 0.1)
            time.sleep(1)
            print(" ")
            speech_manipulation(
                "** You follow the man through a narrow street. After you turn a corner everything is black!", 0.07)
            time.sleep(1)
            clear()
            time.sleep(2)
    ...
    def room_prompt(self):
        ...
        print(" " * (int(
            (len("What would you like to do?") - len("examine, take, put, look, etc.")) / 2)) + "examine, help, etc.")
        print("-" * len("What would you like to do?") + "\n")
        ...
        if len(action) == 1 or len(action) > 2:
            ...
            if action[0] == "turn":
                if action[1] in [o.name for o in turnable]:
                    for obj in turnable:
            ...
            if action[0] == "break":
                if action[1] in [o.name for o in breakable]:
            ...
            if action[0] == "open":
                if action[1] in [o.name for o in openable]:
                    for obj in openable:
                        if action[1] == obj.name:
                            obj.open(self)
                            confirmation()
...
class Notebook():
    ...
    def examine(self, room):
        print(
            "You open the notebook. You read the first lines of the notebook: 'The Earth is the Center.' A piece of paper falls out.")
        room.push_obj(["paper"])
        room.clues_found += 1
...
class Cupboard():
    ...
    def examine(self, room):
        print("An old mahagony wardrobe with big doors. There is a lock on the doors.")

    def open(self, room):
        combination = self.combination
        print("In it you find a note.")
...
class PieceOfPaper():
    ...
    def examine(self, room):
        print(
            "It reads: 'When the planets allign, take the third of the second, the fourth of the fifth, the last of the sixth and the first of the third.")
        room.clues_found += 1
